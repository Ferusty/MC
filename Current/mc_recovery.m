function [eigspect,centerfreq_hat,x_hat,Rz,Phi_Omega]=mc_recovery(y,L,W,c,t,nyqover,type)
...
%*******************
K_hat=24; % for now, choose K_hat manually 
%*******************
...
case 'LS' %% Least Squares Solver Method
s=(W/L)*pinv(Phi_Omega)*z;
%s=(W/L)*mldivide(Phi_Omega,z);
x_hat=sum(s.*exp(1i*2*pi*centerfreq_hat*t(1:size(s,2))),1);

case 'OMP' %% Orthogonal Matching Pursuit
s=(W/L)*MMV_omp(Phi,z,K_hat);
x_hat=sum(s.*exp(1i*2*pi*centerfreq_hat*t(1:size(s,2))),1);

case 'SP' %% Sub-space Pursuit
s=(W/L)*MMV_sp(Phi,z,K_hat);
x_hat=sum(s.*exp(1i*2*pi*centerfreq_hat*t(1:size(s,2))),1);

case 'CSMP' %% CoSaMP
s=(W/L)*MMV_CoSaMP(Phi,z,K_hat);
x_hat=sum(s.*exp(1i*2*pi*centerfreq_hat*t(1:size(s,2))),1);

case 'BP' %% Basis Pursuit
s=(W/L)*spg_mmv(Phi,z,0.00001);
s_abs=abs(sum(s,2));
[s_abs_sort index]=sort(s_abs,'descend');
s_abs_index=index(1:K_hat);
s=s(s_abs_index,:);
x_hat=sum(s.*exp(1i*2*pi*centerfreq_hat*t(1:size(s,2))),1);
