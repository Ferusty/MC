...
K=4; % number of positive disjoint frequency bands present in input signal x
T=20; % duration of observation interval in sec
q=30; % q (uniform) samples collected every L/W seconds (each channel samples W/L Hz)
...
%randn('state',8556);
%s=rng;
%w=sqrt(input_noiseP)*W*B*(randn(s,size(x))+1i*randn(s,size(x))); % generate white Gaussian noise with required variance 
