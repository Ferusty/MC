...
K=2; % number of positive disjoint frequency bands present in input signal x
...
CF= randi([0 500], 20 ,1); % Randomly Generate Frequencies
...
[x,centerfreq,t]=bandsparse(W,B,K,L,nyqover,'FH1H',T,CF,CF2);
...
%randn('state',8556);
%s=rng;
%w=sqrt(input_noiseP)*W*B*2*(randn(s,size(x))+1i*randn(s,size(x))); % generate white Gaussian noise with required variance 
w=sqrt(input_noiseP)*(randn(size(x))+1i*randn(size(x))); % No Noise 
xw=x+w; % add noise
...
%%%% 'SP' Sub-Space Pursuit, 'BP' Basis Pursuit, 'LS' Least Squares, 'OMP' Orthogonal Matching Pursuit, 'CSMP' CoSaMP &&&&   
